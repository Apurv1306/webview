...
AUDIO_FILE: str = "thank_you.mp3"  # Handled by frontend
TICK_ICON_PATH: str = "tick.png"  # Handled by frontend
...
def _crop_and_resize_for_passport(cv_image: np.ndarray, target_size: Tuple[int, int] = (240, 320)) -> np.ndarray:
    """ Crops and resizes an image to a target aspect ratio and size, similar to passport photo requirements. """
...
class ComplimentGenerator:
    def __init__(self):
        self.user_compliment_history: Dict[str, Dict[str, List[str]]] = {}  # emp_id -> {'dates': [], 'compliments': []}
    ...
    def get_daily_compliment(self, emp_id: str, name: str) -> str:
        ...
        # Remove expired compliments (older than today)
        idx_to_remove = [i for i, d in enumerate(self.user_compliment_history[emp_id]['dates']) if d != today]
        for i in reversed(idx_to_remove):  # Remove from end to avoid shifting
            self.user_compliment_history[emp_id]['dates'].pop(i)
            self.user_compliment_history[emp_id]['compliments'].pop(i)

        # Get available compliments (not used for this user today)
        available = [c for c in self.compliment_template
                    if c not in self.user_compliment_history[emp_id]['compliments']]
        if not available:
            # All compliments used today, reset
...
class FaceAppBackend:
    def __init__(self):
        ...
        self.compliment_gen = ComplimentGenerator()  # New: Compliment generator
    ...
    def _train_recognizer(self):
        """ Trains the LBPHFaceRecognizer with images found in known_faces_dir. Returns the trained recognizer and a label map. """
    ...
    def _send_otp_email(self, email: str, otp: str, name: str, emp_id: str, is_admin_email: bool = False) -> bool:
        ...
        if is_admin_email:
            ...
            body_html = (f"<p>Details of a person for whom an OTP process was initiated:</p>"
                         f"<p><strong>Name:</strong> {name.title()}<br/>"
                         f"<strong>Employee ID:</strong> {emp_id}<br/>"
                         f"<strong>Generated OTP:</strong> <strong>{otp}</strong></p>")
        else:
            subject = f"FaceApp: Your OTP for {name.title()} ({emp_id})"
            body_html = (f"<p>Dear {name.title()},<br/><br/>"
                         f"Your OTP is <strong>{otp}</strong>. It is valid for 10 minutes.<br/><br/>"
                         f"Please use this OTP to proceed with your photo update/registration.</p>")
    ...
    def _send_attendance_email(self, email: str, name: str, emp_id: str, detection_time: str, email_type: str, face_image_b64: Optional[str] = None) -> bool:
        """
        Sends an attendance email (in-time or out-time) with a personalized message and optional embedded face image.
        email_type can be "in" or "out".
        """
        current_date_display = datetime.now().strftime("%A, %B %d, %Y")
        image_cid = "detected_face_image"
        image_html = ""
        image_data = None
        ...
        if face_image_b64:
            try:
                ...
                image_html = f'<img src="cid:{image_cid}" alt="Your Photo" style="max-width: 240px;">'
            except Exception as e:
                Logger(f"[ERROR] Failed to decode base64 image for email: {e}")
                image_html = ""
                image_data = None

        # Get a daily-unique compliment for in-time
        compliment = self.compliment_gen.get_daily_compliment(emp_id, name.title()) if email_type == "in" else None
        ...
        if email_type == "in":
            ...
            body_html = (
                ...
                f"<p>{image_html}</p>"
                f"<p><strong>Date:</strong> {current_date_display}</p>"
                f"<p><strong>In-Time:</strong> {detection_time}</p>"
                f"<p>We noticed: {compliment}</p>"
                f"<p>Thank you for being part of our team!</p>"
            )
        elif email_type == "out":
            ...
            body_html = (
                ...
                f"<p>{image_html}</p>"
                f"<p><strong>Date:</strong> {current_date_display}</p>"
                f"<p><strong>Out-Time:</strong> {detection_time}</p>"
                f"<p>Thank you for your support today. Have a great evening!</p>"
            )
        else:
    ...
    def process_frame(self, frame_data_b64: str) -> Dict[str, Any]:
        ...
        If in capture mode, it saves face samples. Returns detection/recognition results.
        """
        try:
            ...
            for (x, y, w_s, h_s) in faces:
                ...
                x_full, y_full, w_full, h_full = [int(v / 0.5) for v in (x, y, w_s, h_s)]
    ...
    def start_capture_samples(self, name: str, emp_id: str, updating: bool = False, sample_count: Optional[int] = None) -> Dict[str, Any]:
        """Initiates the sample capture process. Sets internal flags for process_frame to start saving images."""
        with self.capture_lock:
            if self.capture_mode:
